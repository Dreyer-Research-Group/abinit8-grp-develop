...
interface
 subroutine overlap_k1k2_paw(cprj_k1,cprj_k2,dk,gprimd,k1k2_paw,lmn2max,lmnsize,mband,&  
  ...
  integer,intent(in) :: ntypat
  type(pawang_type),intent(in) :: pawang
  type(pawcprj_type),intent(in) :: cprj_k1(natom,mband)
  type(pawcprj_type),intent(in) :: cprj_k2(natom,mband)
  real(dp),intent(in) :: dk(3)
  real(dp),intent(in) :: gprimd(3,3)
  real(dp),intent(out) :: k1k2_paw(2,mband,mband)
  integer,intent(in) :: lmnsize(ntypat)
  type(pawrad_type),intent(in) :: pawrad(ntypat)
  type(pawtab_type),intent(in) :: pawtab(ntypat)
  integer,intent(in) :: typat(natom)
...
interface
 subroutine pawinit(gnt_option,gsqcut_eff,hyb_range_fock,lcutdens,lmix,mpsang,nphi,nsym,ntheta,&  
  ...
  real(dp),intent(in) :: hyb_range_fock
...
interface
 subroutine qijb_kk(calc_qijb,dkvecs,expibi,gprimd,lmn2max,natom,ntypat,&  
  ...
  real(dp),intent(in) :: dkvecs(3)
  real(dp),intent(in) :: expibi(2,natom)
  real(dp),intent(in) :: gprimd(3,3)
  type(pawrad_type),intent(in) :: pawrad(ntypat)
  type(pawtab_type),intent(in) :: pawtab(ntypat)
  integer,intent(in) :: typat(natom)
