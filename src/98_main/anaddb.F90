...
&     ab_out,lst,mpert,natom,inp%nph2l,phfrq,comm,ana_ncid)
   ...
   ! *******************************************
   ! CEDrev: Lets assemble the S matrix here
   ! *******************************************
!!$
!!$   ! First, find q=0
!!$   do iq=1,Ifc%nqbz
!!$      if (sqrt(dot_product(Ifc%qbz(:,iq),Ifc%qbz(:,iq)))) exit
!!$   end do
!!$
!!$   ! Now assemble the matrix s_mat
!!$   ABI_ALLOCATE(s_mat,(3*Ifc%natom+3,3*Ifc%natom+3))
!!$   s_mat(:,:) = zero
!!$
!!$   ! head: Drude/\epsilon_p
!!$   do ii=1,3
!!$      do jj=1,3
!!$         s_mat(ii,jj)=drude/dielt(ii,jj)
!!$      end do
!!$   end do
!!$
!!$   ! wings: mode BEC
!!$   do ii = 4,3+3*Ifc%natom
!!$      do jj=1,3
!!$         s_mat(ii,jj)=drude/dielt(ii,jj)
!!$      end do
!!$   end do
!!$
!!$   ! body: diagonal, mode eigenvalues
!!$   do ii=4,3*Ifc%natom+3
!!$      s_mat(ii,ii)=eigenval(ii-3)
!!$   end do
