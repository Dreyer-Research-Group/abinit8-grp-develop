...
! CEDrev: 
integer :: velfr,symon, prt_eigen1_dk
integer :: index_k,index_k1,index_bnd,index_bnd2,index_eij,index_eji,npw,npw1,index_kg,index_kg1
integer :: bnd_start,bnd_fin,bnd_start1,bnd_fin1,nbd_max,isp_max,nbd,isp,finq
integer,allocatable :: gbd(:,:),gbd1(:,:),kg_k(:,:),kg1_k(:,:)
real(dp) :: fermi_fac,fermi_fac_dab,fij,deleig,deleig_inv,dotr1,doti1,dotr2,doti2
real(dp) :: eta
real(dp), allocatable :: denpot(:,:,:),dumr(:,:,:,:)
real(dp),allocatable :: cg1_tilde_dk(:,:),eigen1_dk(:),rocceig(:,:)
real(dp),allocatable :: cg1_act_bnd(:,:),cg1_act_bnd_kmesh(:,:),cg1_dk_bnd(:,:)
complex(dp) :: dedw1(2),dedw2(2),dab1(2),dab2(2),dot_tst
...
! CEDrev: Will we be computing force-velocity matrix elements?
velfr=dtset%useria 
! will we reduce kpoints with symmetry? 1: yes, 0:no
symon=0
write(*,*) 'SYMON=1'
prt_eigen1_dk=0 ! 0 = do not print 1st order eigenfunctions 
if (dtset%userie==-1) prt_eigen1_dk=1

! Force printing of eigen1 for Dab
!if (dtset%useric > 0) then
!   prt_eigen1_dk = 1
!end if


!!$! CEDrev: open files for Max's outputs in case of loops over pert/dir
!!$if (velfr == 2 .and. dtset%userie==2) then
!!$
!!$     ! open files for output
!!$     open(unit=file_dcova_dcovb,file='dcova_dcovb.dat',status='replace')
!!$     open(unit=file_uj_Hb_ui,file='uj_Hb_ui.dat',status='replace')
!!$     open(unit=file_ui_Ha_uj,file='ui_Ha_uj.dat',status='replace')
!!$
!!$     write(file_dcova_dcovb,*) '# ipert_a ipert_b idir_a idir_b ikpt wtk kptx kpty kptz band real img'
!!$     write(file_uj_Hb_ui,*) '# ipert_b idir_b ikpt wtk kptx kpty kptz jband iband real img'
!!$     write(file_ui_Ha_uj,*) '# ipert_a idir_a ikpt wtk kptx kpty kptz iband jband real img'
!!$
!!$     close(file_dcova_dcovb)
!!$     close(file_uj_Hb_ui)
!!$     close(file_ui_Ha_uj)
!!$
!!$     if (dtset%useric > 0) then
!!$        open(unit=file_dcova_Hb_ui,file='da_Hb_ui.dat',status='replace')
!!$        open(unit=file_ui_Ha_dcovb,file='ui_Ha_db.dat',status='replace')
!!$        write(file_dcova_Hb_ui,*) '# ipert_a ipert_b idir_a idir_b ikpt wtk kptx kpty kptz band real img'
!!$        write(file_ui_Ha_dcovb,*) '# ipert_a ipert_b idir_a idir_b ikpt wtk kptx kpty kptz band real img'
!!$        close(file_dcova_Hb_ui)
!!$        close(file_ui_Ha_dcovb)
!!$     end if
!!$
!!$end if
...
! finite q?
!if (dot_product(dtset%qptn,dtset%qptn) > 1.0d-8) then
!   finq=1
!else
!   finq=0
!end if
...
! CEDrev: control the symmetry
   if(dtset%ieig2rf>0.or.symon==0) then
...
! NB: 4 Sept 2013: this was introducing a bug - for ieig2rf ==0 the dim was being set to 1 and passed to dfpt_vtowfk
   ...
   if ((dtset%ieig2rf > 0 .and. dtset%ieig2rf/=2) .or. dtset%efmas > 0 .or. velfr > 0) then
...
&     'Note: Compiling with large int (int64) requires a full software stack (MPI/FFTW/BLAS/LAPACK...) compiled in int64 mode'
   ...
   ! CEDrev: FO H from previous pert applied GS wf
   !ABI_ALLOCATE(gh1c_set_prev,(2,mpw1*dtset%nspinor*dtset%mband*mk1mem_rbz*dtset%nsppol*dim_eig2rf))
   ...
   if (velfr == 2) then
      if (.not.allocated(cg1_tilde_dk)) ABI_ALLOCATE(cg1_tilde_dk,(2,mcg1))
      cg1_tilde_dk=0.
...
!CEDrev: only read in wf if velfr == 0
   call inwffil(ask_accurate,cg1,dtset,dtset%ecut,ecut_eff,eigen1,dtset%exchn2n3d,&
&   formeig,hdr,&
&   ireadwf0,istwfk_rbz,kg1,kpq_rbz,dtset%localrdwf,&
...
!    Close wff1 (filename fiwf1i), if it was ever opened (in inwffil)
 ...
 if (velfr == 2) then
    ...
    !TEST
    !write(*,*) "before wfread", fiwf1o_tild

    ! CEDrev: If velfr (and \tau pert), read in previous FO file, which contains
    ! \widetilde{\delta_k\psi} and <i|\delta_k H|j> 
    if (.not.allocated(eigen1_dk)) ABI_ALLOCATE(eigen1_dk,(2*dtset%mband*dtset%mband*nkpt_rbz*dtset%nsppol))
    eigen1_dk=zero
    ! CEDrev:
    ! write(*,*) "before wfread for ",fiwf1i
    call timab(144,1,tsec)
    call status(pertcase,dtfil%filstat,iexit,level,'call inwffil  ')
    call inwffil(ask_accurate,cg1_tilde_dk,dtset,dtset%ecut,ecut_eff,eigen1_dk,dtset%exchn2n3d,&
         &     formeig,hdr,&
         &     ireadwf0,istwfk_rbz,kg1,kpq_rbz,dtset%localrdwf,&
         &     dtset%mband,mcg1,mk1mem_rbz,mpi_enreg,mpw1,nband_rbz,dtset%ngfft,nkpt_rbz,npwar1,&
         &     dtset%nsppol,nsym1,occ_rbz,optorth,&
...
!    do ii=1,mcg1
!       write(19,'(4e20.4e2)') cg1_tilde_dk(:,ii)
!    end do
!    close(unit=19)
!    stop
...
! CEDrev: pass gh1c_set_prev for application of FO H of prev pert to GS wf. For now off
     if (kramers_deg) then
...
&       enl0,enl1,eovl1,epaw1,etotal,evdw,exc1,fermie,gh0c1_set,gh1c_set,& !gh1c_set_prev,
&       hdr,idir,&
...
&       rhog1_pq=rhog1_pq,rhog1_mq=rhog1_mq,rhor1_pq=rhor1_pq,rhor1_mq=rhor1_mq)
     ...
     ! TEST: Compute BEC from FO charge. 

     if (dtset%userib==1) then 
#ifdef HAVE_MPI
        if (mpi_enreg%me_kpt==0) then
           write(*,'(a20,5e18.8e3)') 'G=0 of rhog1 proc', two_pi*matmul(gprimd(:,:),dtset%qptn(:)),matmul(rmet(:,:),rhog1(:,1))
        end if
#endif
     end if
     ...
     if (velfr == 2) call velfrc(cg,cg1_active,cg1_tilde_dk,cplex,doccde_rbz,docckqde,dtfil,dtset,eigen0,eigen1,eigenq,gh1c_set, &
     ...
     if (velfr == 1) then
        ...
        call outwf(cg1_active,dtset,psps,eigen1,fiwf1o_tild,hdr,kg1,kpt_rbz,&
        ...
        if (mpi_enreg%me_kpt==0) then
           ...
           if (dtset%userib==2.and.dtset%userid==0) then ! finite q dk
              write(19,*) dtset%natom+99,dtset%rfdir
           else if(dtset%userib==2.and.dtset%userid==-1) then ! local only
...
! CEDrev: check for velfr
   ...
   if (dtset%prtwf==-1 .and. dfpt_scfcv_retcode == 0 .and. velfr == 0) then
     write_1wfk = .False.
...
!end if
   ...
   ! CEDrev: Extract diagonal elements of eigen1, as above for gkk HERE
!!$   if (dtset%useria>=1.and.me == master) then
!!$
!!$!      write(file_name,'(a7,i5.5,a1,i1,a4)') 'dHdeps_',ipert,"_",idir,".dat"
!!$      write(file_name,'(a7,i5.5,a1,i1,a4)') 'dHdeps_',ipert,"_",dtset%userib,".dat"
!!$
