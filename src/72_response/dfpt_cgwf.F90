...
!CEDrev: pass dtset,psps,kpt
subroutine dfpt_cgwf(band,berryopt,cgq,cwavef,cwave0,cwaveprj,cwaveprj0,rf2,dcwavef,&
& dtset,eig0nk,eig0_kq,eig1_k,ghc,gh1c_n,grad_berry,gsc,gscq,&
& gs_hamkq,gvnlc,gvnl1,icgq,idir,ipert,igscq,kpt,&
& mcgq,mgscq,mpi_enreg,mpw1,natom,nband,nbdbuf,nline_in,npw,npw1,nspinor,&
& opt_gvnl1,prtvol,psps,quit,resid,rf_hamkq,dfpt_sciss,tolrde,tolwfr,&
...
#define ABI_FUNC 'dfpt_cgwf'
 ...
 use interfaces_72_response, except_this_one => dfpt_cgwf
...
!scalars
 ...
 real(dp),intent(in) :: eig0nk,dfpt_sciss,tolrde,tolwfr, kpt(3) ! CEDrev
 real(dp),intent(out) :: resid
 type(MPI_type),intent(in) :: mpi_enreg
 type(rf2_t), intent(in) :: rf2
 type(gs_hamiltonian_type),intent(inout) :: gs_hamkq
 type(rf_hamiltonian_type),intent(inout) :: rf_hamkq
 type(dataset_type), intent(in), target :: dtset ! CEDrev
 type(pseudopotential_type),intent(in) :: psps ! CEDrev
!arrays
...
!arrays
 ...
 real(dp) :: cwave0_npw1(2,npw1*nspinor),cwave0_tmp(2,npw),cwave0_npw1_sp(2,npw1)
...
!  Compute H(1) applied to GS wavefunction Psi(0)
   ...
   ! ***************************************************
   if (dtset%userib==1) then ! Adiabatic perturbation theory NOT FULLY IMP!!
      write(*,*) "ERROR, ADIABATIC NOT IMP"
      stop
      !gh1c(1:2,1:npw1*nspinor) = cwave1in(1:2,1:npw1*nspinor)

   else if (dtset%userib==2) then ! Diamagnetic sucseptability

      ! TODO: Noncolinear, need to multiply every npw by nspinor

      !TEST
      !write(*,'(a5,3e12.3e2,a12,2i10,a30,2i10)') "kpt",kpt,"npw,npw1",npw,npw1,"gs_hamkq%npw_k, npw_kp",gs_hamkq%npw_k,gs_hamkq%npw_kp
      ...
      ! Cartesian q
      qpc(:)=two_pi*matmul(gs_hamkq%gprimd(:,:),dtset%qptn(:))

      ! Need to FFT cwave0 onto npw1 mesh
      !ABI_ALLOCATE(cwave0_npw1,(2,npw1*dtset%nspinor))
      !ABI_ALLOCATE(cwave0_tmp,(2,npw))
      !ABI_ALLOCATE(dumr,(2,gs_hamkq%n4,gs_hamkq%n5,gs_hamkq%n6))
      !ABI_ALLOCATE(denconst,(gs_hamkq%n4,gs_hamkq%n5,gs_hamkq%n6))
      denconst=one
      
      do ispinor=1,nspinor
         !do ipw=1,npw
         cwave0_tmp(:,:)=cwave0(:,1+npw*(ispinor-1):npw*(ispinor))
         !cwave0_tmp(1,ipw+npw*(ispinor-1))=cwave0(1,(ispinor-1)*npw+ipw)
         !cwave0_tmp(2,ipw+npw*(ispinor-1))=cwave0(2,(ispinor-1)*npw+ipw)
         !end do

         ! Need to fft one spinor component at a time
         call fourwf(1,denconst,cwave0_tmp,cwave0_npw1_sp,dumr,gs_hamkq%gbound_k,gs_hamkq%gbound_kp,&
              ...
              &   npw,npw1,gs_hamkq%n4,gs_hamkq%n5,gs_hamkq%n6,2,mpi_enreg%paral_kgb,0,one,one)

         cwave0_npw1(:,1+npw1*(ispinor-1):npw1*(ispinor))=cwave0_npw1_sp(:,:)

      end do

         ! Apply current operator
         call joper(dtset%userid+1,cwave0_npw1,dtset,gs_hamkq%gprimd,idir,gs_hamkq%kg_kp,kpt,mpi_enreg,npw1,& 
              & psps,qpc,dtset%rprim_orig(1:3,1:3,1))
         !NOTE: default (userid=0) is local+nl. Setting userid -1 means that calcnl=0, so only local part
         
         !do ipw=1,npw1
         gh1c(:,:)=cwave0_npw1(:,:)
            !gh1c(1,(ispinor-1)*npw1+ipw)=cwave0_npw1(1,ipw)
            !gh1c(2,(ispinor-1)*npw1+ipw)=cwave0_npw1(2,ipw)
         !end do
...
!$OMP PARALLEL DO
 ...
 else ! In 2nd order case, dcwavef/=0 even in NC, and it is already computed in rf2_init (called in dfpt_vtowfk.F90)
   do ipw=1,npw1*nspinor
