...
!    iband->band, nband_k->nband, npw_k->npw, npw1_k->npw1
     ...
     if ( (ipert/=natom+10 .and. ipert/=natom+11) .or. abs(occ_k(iband))>tol8 ) then
        ...
        !CEDrev: Need dtset, psps
       call dfpt_cgwf(iband,dtset%berryopt,cgq,cwavef,cwave0,cwaveprj,cwaveprj0,rf2,dcwavef,&
&       dtset,eig0nk,eig0_kq,eig1_k,gh0c1,gh1c_n,grad_berry,gsc,gscq,gs_hamkq,gvnlc,gvnl1,icgq,&
&       idir,ipert,igscq,dtset%kptns(:,ikpt),mcgq,mgscq,mpi_enreg,mpw1,natom,nband_k,dtset%nbdbuf,dtset%nline,&
&       npw_k,npw1_k,nspinor,opt_gvnl1,prtvol,psps,quit,resid,rf_hamkq,dtset%dfpt_sciss,dtset%tolrde,&
...
! CEDrev: Here, we would like to print/store the FO wfunction
! orthogonalized to GS
...
! CEDrev: cwavef is deallocated here
