...
interface
 subroutine dfpt_init_mag1(idir,rhor1,rhor0,cplex,nfft,nspden,vxc0,kxc0,nkxc)
  ...
  real(dp), intent(in) :: rhor0(nfft,nspden)
  real(dp), intent(out) :: rhor1(cplex*nfft,nspden)
  real(dp), intent(in) :: vxc0(nfft,nspden)
...
interface
 subroutine dfpt_scfcv(atindx,blkflg,cg,cgq,cg1,cg1_active,cplex,cprj,cprjq,cpus,&  
  ...
  real(dp),optional,intent(out) :: residm_mq
  integer,intent(in) :: ngfftf(18)
  integer,intent(in) :: atindx(dtset%natom)
  ...
  real(dp),optional,intent(inout) :: cg1_mq(2,mpw1_mq*dtset%nspinor*dtset%mband*mk1mem*dtset%nsppol)
  ...
  integer,intent(in) :: npwar1(nkpt_rbz)
  integer,optional,intent(in) :: npwar1_mq(nkpt_rbz)
  integer,intent(in) :: npwarr(nkpt_rbz)
  ...
  real(dp),optional,intent(out) :: resid_mq(dtset%mband*nkpt_rbz*nspden)
  real(dp),intent(in) :: rhog(2,nfftf)
  real(dp),intent(inout) :: rhog1(2,nfftf)
  real(dp),optional,intent(inout) :: rhog1_mq(2,nfftf)
  real(dp),optional,intent(inout) :: rhog1_pq(2,nfftf)
  real(dp),intent(in) :: rhor(nfftf,nspden)
  real(dp),intent(inout) :: rhor1(cplex*nfftf,nspden)
  real(dp),optional,intent(inout) :: rhor1_mq(cplex*nfftf,nspden)
  real(dp),optional,intent(inout) :: rhor1_pq(cplex*nfftf,nspden)
  real(dp),intent(in) :: rprimd(3,3)
  integer,intent(in) :: symaf1(nsym1)
  integer,intent(in) :: symrc1(3,3,nsym1)
  integer,intent(in) :: symrl1(3,3,nsym1)
...
interface
 subroutine joper(calcnl,cwave0_npw1,dtset,gprimd,irfdir,kg1_k,kpt,mpi_enreg,npw1,psps,qpc,rprimd)
  ...
  use defs_datatypes
  implicit none
  integer,intent(in) :: calcnl
  integer,intent(in) :: irfdir
  integer,intent(in) :: npw1
  type(dataset_type), intent(in), target :: dtset
  type(mpi_type), intent(in) :: mpi_enreg
  type(pseudopotential_type), intent(in) :: psps
  real(dp), intent(inout) :: cwave0_npw1(2,npw1*dtset%nspinor)
  real(dp), intent(in) :: gprimd(3,3)
  integer, intent(in) :: kg1_k(3,npw1)
  real(dp), intent(in) :: kpt(3)
  real(dp), intent(in) :: qpc(3)
  real(dp), intent(in) :: rprimd(3,3)
...
interface
 subroutine velfrc(cg,cg1_active,cg1_active_prev,cplex,doccde,docckqde,dtfil,dtset,eigen0,eigen1,eigenq,gh1c_set,&  
  ...
  implicit none
  integer,intent(in) :: cplex
  integer,intent(in) :: idir_b
  integer,intent(in) :: ipert_in
  ...
  integer,intent(in) :: nkpt
  integer,intent(in) :: pertcase
  integer,intent(in) :: prt_eigen1_dk
  integer,intent(in) :: usecprj
  integer,intent(in) :: useylmgr1
  type(datafiles_type), intent(in) :: dtfil
  type(dataset_type), intent(in), target :: dtset
  ...
  real(dp),intent(in) :: docckqde(dtset%mband*nkpt*dtset%nsppol)
  real(dp),intent(in) :: eigen0(dtset%mband*nkpt*dtset%nsppol)
  real(dp),intent(in) :: eigen1(2*dtset%mband*dtset%mband*nkpt*dtset%nsppol)
  real(dp),intent(in) :: eigenq(dtset%mband*nkpt*dtset%nsppol)
  real(dp),intent(in) :: gh1c_set(2,mpw1*dtset%nspinor*dtset%mband*mk1mem*dtset%nsppol)
  real(dp),intent(in) :: gmet(3,3)
  real(dp),intent(in) :: gprimd(3,3)
  integer,intent(in) :: istwfk(nkpt)
  integer,intent(in) :: kg(3,mpw*nkpt)
  integer,intent(in) :: kg1(3,mpw1*nkpt)
  integer,intent(in) :: nband(nkpt*dtset%nsppol)
  integer,intent(in) :: npwar1(nkpt)
  integer,intent(in) :: npwarr(nkpt)
  real(dp),intent(in) :: occ(dtset%mband*nkpt*dtset%nsppol)
  real(dp),intent(in) :: occkq(dtset%mband*nkpt*dtset%nsppol)
